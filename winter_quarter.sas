...
data anxdep01;
	set Sewa;
	where topic = 'Anxiety/Depression';
	pre_correct = 0;
	post_correct = 0;
	if pre_q1 = 'C' then pre_correct = pre_correct + 1;
	if post_q1 = 'C' then post_correct = post_correct + 1;
	if pre_q2 = 'C' then pre_correct = pre_correct + 1;
	if post_q2 = 'C' then post_correct = pos	L;correct + 1;
	\\\\\\\\\\\\\\\\\\\\npre_correct = pre_correct + 1;
	if post_q3 = 'D' then post_correct = post_correct + 1;
